...
public class BuridBlockSearchCluster<E extends Human> extends MapClusterType<E> {
	...
	public BuridBlockSearchCluster(SOSAgent me, List<E> agents) {
		super(me, agents);
	}
...
//		return makeClustersEkmeans(size);
	...
	private void assignSpecialTaskPoliceCluster(final SOSWorldModel model) {
		...
		for (PoliceForceTask pt : ((PoliceWorldModel) me.model()).getPoliceForSpecialTask()) {
			clusters.put((E) pt.getRealEntity(), tmp.remove(0));
		}
	...
	private int getNumValidAgents() {
		...
		if (me instanceof PoliceForceAgent)
			numValidAgents -= ((PoliceWorldModel) me.model()).getPoliceForSpecialTask().size();
	...
	private List<E> getValidAgents(List<E> agents) {
		...
		if (me instanceof PoliceForceAgent)
			removePoliceSpecialTasks(result);
		if (isNoCommunication())//NoComunication
	...
	private void removePoliceSpecialTasks(List<E> result) {
		for (int i = result.size() - 1; i > -1; i--) {
			for (PoliceForceTask t : ((PoliceWorldModel) me.model()).getPoliceForSpecialTask()) {
				if (result.get(i).getID().equals(t.getRealEntity().getID())) {
					log().debug(result.get(i) + " was removed because it is police special task");
					result.remove(i);
					break;
				}
			}
	...
	private ClusterData chooseNearestRegion(ArrayList<ClusterData> regions, E agent) { //Salim
		...
		if (!(agent instanceof PoliceForce && (((PoliceWorldModel) me.model()).getPoliceForSpecialTask().contains(((PoliceWorldModel) me.model()).getPoliceTasks((PoliceForce) agent))))) {
			regions.remove(best);
		}
