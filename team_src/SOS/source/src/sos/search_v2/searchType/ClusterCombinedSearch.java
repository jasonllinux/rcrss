 ...
 * @param <E>
 */
public class ClusterCombinedSearch<E extends Human> extends SearchStrategy<E> {

	public ClusterCombinedSearch(SOSAgent<E> me, SearchWorldModel<E> searchWorld, AgentSearchScore scoreFunction, AgentSearch<?> agentSearch) throws InstantiationException, IllegalAccessException {
	...
	public SearchTask searchTask() {
		log("Cluster Combine Search.......................................");
		double weight = 0;
		if (target != null)
			weight = getWeightTo(target);
		if (target == null || isTargetDone() || weight > searchTools.getWeightIncreaseCoef(me) * lastWeight) {
			target = chooseTarget();
			if (target == null)
				return null;
	...
	public SearchBuilding chooseTarget() {
		...
		log("Cluster Combine Search Choose Task done: " + (System.currentTimeMillis() - t1));
	...
	private boolean isTargetDone() {
		...
		if (seeInSide(target)) {
			log("Target: " + target + " see in side===>target done...");
			return true;
		}
		if (!(me instanceof PoliceForceAgent) && me.move.isReallyUnreachable(target.getRealBuilding().getSearchAreas())) {
			log("Target: " + target + " I am not a police and i was not reachable");
			return true;
		}
		if (!target.needsToBeSearchedForCivilian() && target.isSearchedForFire()) {
			log("Target: " + target + " is searched for civilian and searched for fire");
			return true;
		}
	...
	public PriorityQueue<SearchBuilding> scoringList(Collection<Building> all) {
		...
		FOR: for (Building element : all) {
			...
			if (cost > MoveConstants.UNREACHABLE_COST) {
				b.addScore("not reachibility ", AgentSearchScore.SEARCH_FILLTER_SCORE);
				continue FOR;
			}
			if (cost > 15)
	...
	public SearchType getType() {
		return SearchType.ClusterCombinedSearch;
	}
