...
public class AmbulanceTeamAgent extends AbstractAmbulanceTeamAgent {
	...
	protected void prepareForThink() {
		...
		//		setToOthersTargetIfItAssignACivilianThatAnotherAmbulanceIsWorkingOn();
	}

	//	private void handleHumanChange() {
	//
	//		for (Human current :getVisibleEntities(Human.class)) {
	//				current.getRescueInfo().getPartileFilter().setDmg(current.getDamage(), time());
	//				current.getRescueInfo().getPartileFilter().setBury(current.getBuriedness());
	//				current.getRescueInfo().getPartileFilter().setHp(current.getHP(), time());
	//
	//			}
	//	}

	/*
	 * private void setToOthersTargetIfItAssignACivilianThatAnotherAmbulanceIsWorkingOn() {
	 * for (EntityID entity : lastChangeSet.getChangedEntities()) {
	 * StandardEntity ent = model().getEntity(entity);
	 * if(ent instanceof AmbulanceTeam){
	 * if(((AmbulanceTeam) ent).getPositionArea().equals(me().getPositionArea())){
	 * }
	 * }
	 * }
	 * }
	...
	private void finishTasksState() throws SOSActionException {
		if (time() % 5 == 0)
			sendInfoMessage();
	...
	private void checkIgnored() {
		...
		for (Human hu : lastCycleSensedHumans)
			if (hu.isPositionDefined() && hu.getPosition() instanceof Road) {
				log().info("in checkIgnored function in AmbulanceTeamAgent class");
				if (hu.getRescueInfo().isIgnored() && (hu.getRescueInfo().getIgnoredUntil() >= time() + 5)) {
	...
	private HashMap<AmbulanceTeam, ArrayList<Human>> getSensedAmbulanceAndTargets() {
		...
		for (Human hum : sensedHum) {
			if (AmbulanceUtils.isValidToDecide(hum, log())) {
...
//			hm.getRescueInfo().getPartileFilter().setDmg(hm.getDamage(), time());
//			hm.getRescueInfo().getPartileFilter().setBury(hm.getBuriedness());
//			hm.getRescueInfo().getPartileFilter().setHp(hm.getHP(), time());
		}
...
//				int buriedness = data.get(DATA_BURIEDNESS);
//				int time = data.get(DATA_TIME);
//				h.getRescueInfo().getPartileFilter().setBury(buriedness);
//				h.getRescueInfo().getPartileFilter().setDmg(dmg, time);
//				h.getRescueInfo().getPartileFilter().setHp(hp, time);
				if (civilianFlag) {
