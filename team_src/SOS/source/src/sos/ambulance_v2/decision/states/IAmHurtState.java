...
public class IAmHurtState extends SOSIState {
		...
		if(!isValid()){
		 ...
		 if(infoModel.getModel().refuges().isEmpty()){
			 if(infoModel.getATEntity().getWork() != null &&  infoModel.getATEntity().getWork().getTarget() != null){
				 ...
				 target.getRescueInfo().setIgnoredUntil(IgnoreReason.InvalidOldTarget, 1000);
	...
	private boolean iWillDieInNextCycles(){
		...
		int rescueTime = (infoModel.getModel().refuges().isEmpty())?1000 : getTimeToNearestRefuge()+((AmbulanceTeamAgent)infoModel.getAgent()).me().getBuriedness();
