...
import sos.ambulance.decision.AmbulanceDecision;
...
public class AmbulanceUtils {
	...
	public static boolean isValidToDecide(Human hm, SOSLoggerSystem log) {
	...
	public static boolean isValidToDecide(VirtualCivilian virtualcivilian, AmbulanceTeam AT, SOSLoggerSystem log) {
	...
	public static int taskAssigningExpireTime(Human hm, int deathTime) {
		...
		expire += AmbulanceDecision.AVERAGE_MOVE_TO_TARGET;
	...
	//-------------------------------------------------------------------------------------
	public static void updateATtarget(Human target, AmbulanceTeam agent, SOSIState state) {
		...
		((AmbulanceTeamAgent) agent.getAgent()).setUnderMissionTarget(target);
		//		target.getRescueInfo().addAT(agent);
	...
	public static void sendRejectMessage(Human target, AmbulanceTeamAgent ambulance) {
		...
		ambulance.logToAmbDecision("sending Task Ack Message" + target + "==> rejected");
	}
	...
	public static void rejectTarget(Human target, AmbulanceTeam me, AmbulanceTeamAgent ambulance) throws SOSActionException {
		...
		ambulance.setUnderMissionTarget(null);
		if (me.getWork().getTarget().equals(target))
			me.getWork().setTarget(null, null);
		ambulance.currentSaveHumanTask = null;
