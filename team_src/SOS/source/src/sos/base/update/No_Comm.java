...
public class No_Comm implements MessageXmlConstant {
	...
	private void chooseRoadMessages() {// TODO choose to spread it over time
		...
		PriorityQueue<Road> roads = new PriorityQueue<Road>(agent.model().roads().size(), new Comparator<Road>() {
...
//					continue FOR;
	...
	private void chooseFireMessages() {
			...
			public int compare(Building o1, Building o2) {
				if (o1.getFieryness() != o2.getFieryness()) {
					...
					if (o2.getFieryness() == 2)
						return 1;
					if (o1.getFieryness() == 3)
						return -1;
					if (o2.getFieryness() == 3)
						return 1;
				}
				return 0;
		...
		for (Building b : buildings) {
			...
			if (agent.model().time() - b.updatedtime() < 100) {
				...
				CommunicationlessSearch<?> nocommSearch = b.model().sosAgent().newSearch.strategyChooser.noCommunication;
				if (nocommSearch.getCenteralMan() != null && nocommSearch.getCenteralMan().equals(b.model().me())) {
					sayMessages.add(messageBlock);
				} else {
	...
	public void chooseAllMessages() {
		...
		model().sosAgent().sosLogger.act.debug(" choosing say messages got:" + (System.currentTimeMillis() - t) + "ms");
