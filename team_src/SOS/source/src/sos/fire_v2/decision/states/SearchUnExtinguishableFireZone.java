...
public class SearchUnExtinguishableFireZone extends SOSIState<FireInformationModel> {
	...
	public SOSTask<?> decide(SOSEventPool eventPool) {
		...
		if (ef.getDangerBuildingForIgnit().size() == 0) {
			ef.setDisable(true, infoM.getTime());
			return null;
		}
	...
	private boolean isSearcherAbsent(SOSEstimatedFireZone sosEstimatedFireZone) {
		...
		for (int i : x) {
			try {
				if (fireZoneSelector.zone_FireBrigade.get(i).get(0).updatedtime() > 1)
					if (infoModel.getTime() - fireZoneSelector.zone_FireBrigade.get(i).get(0).updatedtime() < 20)
						if (fireZoneSelector.zone_FireBrigade.get(i).get(0).distance(sosEstimatedFireZone) > Math.max(200000, 1 / 4d * infoModel.getModel().getBounds().getWidth()))
							return true;
