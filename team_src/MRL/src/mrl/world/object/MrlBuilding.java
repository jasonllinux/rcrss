...
public class MrlBuilding {
    public static Map<EntityID, Map<EntityID, MrlBuilding>> VIEWER_BUILDINGS_MAP = new HashMap<EntityID, Map<EntityID, MrlBuilding>>();
...
//            else
    ...
    public double getTempRadiationEnergy() {
        double t = getTempEstimatedTemperature() + 293; // Assume ambient temperature is 293 Kelvin.
        double radEn = (t * t * t * t) * totalWallArea * RADIATION_COEFFICIENT * STEFAN_BOLTZMANN_CONSTANT;
        if (radEn == Double.NaN || radEn == Double.POSITIVE_INFINITY || radEn == Double.NEGATIVE_INFINITY)
            radEn = Double.MAX_VALUE * 0.75;
        if (radEn > tempEnergy) {
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            try {
                if (world.getPlatoonAgent() != null) {
                    if (!MrlWorld.BLOCKED_BUILDINGS.containsKey(world.getPlatoonAgent().getID())) {
                        MrlWorld.BLOCKED_BUILDINGS.put(world.getPlatoonAgent().getID(), new HashSet<EntityID>());
                    }
                    if (reachable) {
                        MrlWorld.BLOCKED_BUILDINGS.get(world.getPlatoonAgent().getID()).remove(getID());
                    } else {
                        MrlWorld.BLOCKED_BUILDINGS.get(world.getPlatoonAgent().getID()).add(getID());
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
