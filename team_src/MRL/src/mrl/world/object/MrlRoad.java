...
import mrl.viewer.layers.MrlRoadLayer;
...
public class MrlRoad {
    ...
    public static Map<EntityID, Map<EntityID, MrlRoad>> VIEWER_ROADS_MAP = new HashMap<EntityID, Map<EntityID, MrlRoad>>();
    ...
    private Road parent;
    private List<EntityID> visibleFrom;
    ...
    private void initialize(Road road, MrlWorld world, List<MrlEdge> mrlEdges) {
        setVisibleFrom(new ArrayList<EntityID>());
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            if (world.getPlatoonAgent() != null) {
                if (!MrlRoadLayer.MRL_ROADS_MAP.containsKey(world.getPlatoonAgent().getID())) {
                    MrlRoadLayer.MRL_ROADS_MAP.put(world.getPlatoonAgent().getID(), new HashMap<EntityID, MrlRoad>());
                }
                MrlRoadLayer.MRL_ROADS_MAP.get(world.getPlatoonAgent().getID()).put(road.getID(), this);
            }
        }
    }
...
//            updateBlockadesValue();
    ...
    private void setBlockadesFromViewer() {
        System.out.println("REMOVE IT(From MrlRoad.class)");
...
//                if (Util.distance(blockade1.getPolygon(), blockade2.getPolygon()) < MRLConstants.AGENT_PASSING_THRESHOLD) {
    ...
    public void resetOldPassably() {
        if (!isSeen() || world.getPlatoonAgent() == null || world.getPlatoonAgent() instanceof MrlPoliceForce) {
            return;
        }
        if (isTimeToReset()) {
            reset();
    ...
    private boolean isTimeToReset() {
        int resetTime = getRepairTime() + MRLConstants.ROAD_PASSABLY_RESET_TIME;
    ...
    public boolean isNeedUpdate() {
        ...
        if ((parent.isBlockadesDefined() && parent.getBlockades().size() != getMrlBlockades().size()) ||
    ...
    public List<EntityID> getVisibleFrom() {
        return visibleFrom;
    }

    public void setVisibleFrom(List<EntityID> visibleFrom) {
