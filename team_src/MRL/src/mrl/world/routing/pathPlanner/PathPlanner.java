...
//        }
    ...
    public void update() {
        /**
         * dar har cycle har roadi ro ke bebine update mikone.
         * yani age grid nadashte bashe grid bandi ba'd har edgesh ro chek mikone ke aya passable hast ya na.
         */
    }
...
//        if (target instanceof Building && !force) {
//            if (!(platoonAgent instanceof MrlPoliceForce) && !world.getMrlBuilding(target.getID()).isOneEntranceOpen(world)) {
...
//        /**
//         * aval chand ta az nazdiktarin refuge haro peyda mikonim.
//         * ba'd ba hame oon ha path mizanim.
//         * pathe nakdiktarin ro barmigardunim.
//         */
//        List<EntityID> finalPath = new ArrayList<EntityID>();
//
//        if (world.getRefuges().size() == 0 || sourceArea instanceof Refuge) {
//            // age refuge nadashte bashim ke hichi.
//            return finalPath;
//        }
//        if (repeatPlanning(null) || !isPositionOnPreviousPath(sourceArea.getID())) {
//            List<EntityID> bestRefugePath = new ArrayList<EntityID>();
//            Area bestRefuge = null;
//            int minimumCost = Integer.MAX_VALUE;
//            List<EntityID> path;
//            int cost;
//            previousPath.clear();
//
//            List<Refuge> nearestRefuges;
//            // te'dadi az refuge haye nazdik (az nazare fasele oghlidosi) ro migire.
//            nearestRefuges = getSomeNearRefuge(sourceArea.getX(), sourceArea.getY());
//
//            for (Area refuge : nearestRefuges) {
//                if (refuge == null) {
//                    continue;
//                }
//
//                if (sourceArea.equals(refuge)) {
//                    Logger.warn("Time:" + world.getTime() + " Already on target move =" + refuge);
//                    return null;
//                }
//
//                // be hameye refuge haye nazdik path mizane va nazdiktarin ro entekhab mikone.
////                path = a_star.getShortestGraphPath(sourceArea, refuge, force);
//                path = planMove(refuge, IN_TARGET, force);
//
//                // kamtarin cost ro peyda mikonim.
//                if (path.size() != 0) {
//                    cost = a_star.getPathCost();
...
//                        minimumCost = cost;
//                        bestRefuge = refuge;
//                        bestRefugePath = new ArrayList<EntityID>(path);
//                    }
//                } else {
//                    setUnreachableTarget(refuge);
//                }
//            }
//
//            if (bestRefuge == null && !nearestRefuges.isEmpty()) {
//                // age refuge khubi peyda nakard avalin refuge ke az nazare oghlidosi nazdike ro entekhab mmikone.
//                if (MRLConstants.DEBUG_PLANNER) {
//                    world.printData(" true move....");
//                }
//                bestRefuge = nearestRefuges.get(0);
////                bestRefugePath = a_star.getShortestGraphPath(sourceArea, bestRefuge, true);
//                bestRefugePath = planMove(bestRefuge, IN_TARGET, force);
//
//            }
//            if (bestRefuge != null) {
//                if (MRLConstants.DEBUG_PLANNER) {
//                    world.printData("  best:" + bestRefuge + " -------- nearest REFUGE = " + nearestRefuges);
//                }
//                previousTarget = bestRefuge;
//                nearestAreaPathCost = minimumCost;
//                previousPath = bestRefugePath;//a_star.getAreaPath(sourceArea.getID(), bestRefuge.getID(), bestRefugePath);
//
//            }
//
//        } else {
//            // age hanooz be refuge nareside bood area haei ke ta inja umade az path hazf mikone va dobare edame mide.
//            ArrayList<EntityID> toRemove = new ArrayList<EntityID>();
//
//            for (EntityID aPreviousPath : previousPath) {
//                if (!sourceArea.getID().equals(aPreviousPath)) {
...
//        }
        ...
        for (int i = 0; i < plan.size() - 1; i++) {//these lines are for test a_star validity.
            ...
            if (edge == null) {
//                    planMove(target, IN_TARGET, false);
                if (LAUNCH_VIEWER) {
                    world.printData("Illegal move plan!!!..............");
                }
                if (illegalPlanCount < 3) {
                    illegalPlanCount++;
                    return;
                } else {
                    platoonAgent.randomWalk();
                    illegalPlanCount = 0;
                    throw new CommandException("random_walk");
                }
            }
        }
        illegalPlanCount = 0;
//        target = world.getEntity(plan.get(plan.size() - 1), Area.class);
        if (escapeHere.amIMotionLess()) {
            ...
            if (state.equals(EscapeState.UNREACHABLE)) {
                ...
                EntityID previousPosition = previousPositionsQueue.remove();
