...
/**
 * This implementation uses different distance parameters (such as distance form refuge to target, distance from agent to target and distance from partition to target)to select best target and uses possible filters such as dead time filters
 */
    ...
    private AmbulanceTarget findBestVictim(Map<EntityID, AmbulanceTarget> targetsMap) {
        ...
        if (targetsMap != null && !targetsMap.isEmpty()) {
            ...
            for (AmbulanceTarget target : targetsMap.values()) {
                if (target.getDistanceToMe() <= rescueRange || myPartition.contains(target.getPositionID(), world) || Util.distance(myPartition.getPolygon(), world.getEntity(target.getPositionID()).getLocation(world)) < rescueRange) {
...
//                        minValue = target.getCost();
     ...
     * this agent partition, distance of this agent to the target and situations of the target based on its BRD and DMG
     *
        ...
        if (victims != null && !victims.isEmpty()) {
            for (StandardEntity victim : victims) {
                ...
                if (target == null) {
                    ...
                    //euclidean distance from this victim to the center of my partition
                    if (myPartition == null) {
                        System.out.println("ajnsiuabnlsi");
                    }
                    if (myPartition.getCenter() == null) {
                        System.out.println("nauliui");
                    }
                    target.setDistanceToPartition(Util.distance(human.getLocation(world), myPartition.getCenter()));
                }
    ...
    private int computingDistance(Human human) {
        ...
        return (int) (coefficient * Util.distance(human.getLocation(world), world.getSelfLocation()));
    }
