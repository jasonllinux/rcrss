...
import mrl.mosCommunication.helper.AmbulanceMessageHelper;
    ...
    private void postActs() throws CommandException {
        ...
        if (myTarget == null) {
            ...
            if (MRLConstants.LAUNCH_VIEWER) {
                MrlHumanLayer.AMBULANCE_TARGETS_TO_GO_MAP.put(me().getID(), null);
            }
            searching(true);
        } else {
            if (MRLConstants.LAUNCH_VIEWER) {
                MrlHumanLayer.AMBULANCE_TARGETS_TO_GO_MAP.put(me().getID(), myTarget.getPosition());
            }
