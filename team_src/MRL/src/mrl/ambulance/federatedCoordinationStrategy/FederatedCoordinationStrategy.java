...
import mrl.viewer.layers.MrlPreRoutingPartitionsLayer;
        ...
        for (int i = 0; i < numOfPartitionPart; i++) {
            for (int j = 0; j < numOfAT.get(i); j++) {
                if (me.getID().equals(halfOfAT.get(i).get(j).getID())) {
                    ...
                    world.getViewerPartitionVictimBuildings().clear();
                    for (Human human1 : victims.get(i)) {
                        world.getViewerPartitionVictimBuildings().add(human1.getPosition());
                    }
                    world.viewerPartitionVisitedBuildings.addAll(visitedBuilding.get(i));
                }
            }
...
//                for (EntityID entityID : myCurrentArea.getNeighbours()) {
