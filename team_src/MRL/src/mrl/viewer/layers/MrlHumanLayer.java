...
public class MrlHumanLayer extends StandardEntityViewLayer<Human> {
    ...
    public void initialise(Config config) {
        ...
        SAY_RANGE = config.getIntValue(Channels.COMMUNICATIONS_CHANNELS_KEY + Channels.VOICE_CHANNEL + Channels.RANGE_KEY);
    ...
    public Shape render(Human h, Graphics2D g, ScreenTransform t) {
        ...
        if (icon == null) {
            ...
            if (h == StaticViewProperties.selectedObject) {
                ...
                if (allAmbulanceTargetsToGo) {
                    renderAllTargetsToGo(g, t, AMBULANCE_TARGETS_TO_GO_MAP);
                }

                if (allFireBrigadeTargetsToGo) {
            ...
            if (showCivilianPosition) {
                if (StaticViewProperties.selectedObject != null &&
                        MrlWorld.UNDEFINED_POSITION_CIVILIANS.containsKey(StaticViewProperties.selectedObject.getID()) &&
                        MrlWorld.UNDEFINED_POSITION_CIVILIANS.get(StaticViewProperties.selectedObject.getID()).contains(h.getID())) {
                    ...
                    circle2D.draw(g);
                        ...
                        MrlWorld.DEFINED_POSITION_CIVILIANS.containsKey(StaticViewProperties.selectedObject.getID()) &&
                        MrlWorld.DEFINED_POSITION_CIVILIANS.get(StaticViewProperties.selectedObject.getID()).contains(h.getID())) {
                    Circle2D circle2D = new Circle2D(t.xToScreen(h.getX()), t.yToScreen(h.getY()), 13d, true);
                    g.setColor(Color.GREEN);
                    circle2D.draw(g);
                }
