 ...
 * Time: 10:15:13 AM
 */
public class MrlUnvisitedBuildingLayer extends MrlAreaLayer<Building> {
    ...
    public static Map<EntityID, List<EntityID>> UNVISITED_BUILDINGS_MAP = Collections.synchronizedMap(new FastMap<EntityID, List<EntityID>>());


    /**
     * Construct a building view layer.
     */
    public MrlUnvisitedBuildingLayer() {
        super(Building.class);
    }

    @Override
    public String getName() {
        return "UnVisited Buildings";
    }
    ...
    protected void paintShape(Building b, Polygon shape, Graphics2D g) {
        if (StaticViewProperties.selectedObject != null) {
            ...
            List<EntityID> unvisited = null;
            try {
                unvisited = Collections.synchronizedList(UNVISITED_BUILDINGS_MAP.get(StaticViewProperties.selectedObject.getID()));
            } catch (NullPointerException ignored) {
            }

            if (unvisited != null && unvisited.contains(b.getID())) {
