 ...
 * Time: 5:51 PM
 */
public class MrlZIOLayer extends MrlAreaLayer<Building> {
    ...
    public static Map<EntityID, List<MrlBuilding>> SELECTED_ZONE_MAP = Collections.synchronizedMap(new FastMap<EntityID, List<MrlBuilding>>());
...
//                t.yToScreen(e.getEndY()));
    ...
    protected void paintShape(Building b, Polygon shape, Graphics2D g) {
        if (StaticViewProperties.selectedObject != null) {
            ...
            try {
                buildings = Collections.synchronizedList(SELECTED_ZONE_MAP.get(StaticViewProperties.selectedObject.getID()));
...
//        drawFieriness(b, shape, g);
    ...
    private void drawBuildings(Shape shape, Graphics2D g) {
