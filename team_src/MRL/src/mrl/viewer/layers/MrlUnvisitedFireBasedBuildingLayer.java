...
public class MrlUnvisitedFireBasedBuildingLayer extends MrlAreaLayer<Building> {
    ...
    public static Map<EntityID, List<Building>> UNVISITED_FIRE_BASED_BUILDINGS_MAP = Collections.synchronizedMap(new FastMap<EntityID, List<Building>>());
    ...
    protected void paintShape(Building b, Polygon shape, Graphics2D g) {
        if (StaticViewProperties.selectedObject != null) {
            ...
            List<Building> unvisited = null;
            try {
                unvisited = Collections.synchronizedList(UNVISITED_FIRE_BASED_BUILDINGS_MAP.get(StaticViewProperties.selectedObject.getID()));
