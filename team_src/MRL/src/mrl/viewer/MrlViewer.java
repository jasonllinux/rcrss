...
import mrl.viewer.layers.MrlLocationLayer;
 ...
 * A simple viewer.
 */
public class MrlViewer extends StandardViewer implements MRLConstants {

    public static Map<EntityID, List<Integer>> civilianPassableBuildingMap = new FastMap<EntityID, List<Integer>>();
    public static Map<EntityID, MrlBuilding> TEMP_BUILDING_MAP = new FastMap<EntityID, MrlBuilding>();
    ...
    Map<Integer, KVTimestep> dataMap = new FastMap<Integer, KVTimestep>();
    ...
    public void selectObject(Integer id) {
        ...
        MrlPlatoonAgent.CHECK_ID = new EntityID(id);
        ...
        propertiesTable.setValueAt(selectedObject.getProperty(X_URN).getValue(), i++, 1);
        propertiesTable.setValueAt("y", i, 0);
        propertiesTable.setValueAt(selectedObject.getProperty(Y_URN).getValue(), i++, 1);
        ...
        if (selectedObject instanceof Human) {
            ...
            if (selectedObject instanceof Civilian) {
                ...
                MrlPlatoonAgent platoonAgent = MrlPlatoonAgent.PLATOON_AGENTS_FOR_VIEWER.get(selectedObject.getID());
                if (platoonAgent != null) {
                    ...
                    propertiesTable.setValueAt(CivilianSearchDecisionMaker.AGENT_TARGET.get(platoonAgent.getID()), i++, 1);
...
//                }
//            }
            EntityID agent = ((StandardEntity) agentCombo.getSelectedItem()).getID();
            if (selectedObject instanceof Building) {
                ...
                propertiesTable.setValueAt("fieryness", i, 0);
                propertiesTable.setValueAt(selectedObject.getProperty(FIERYNESS_URN).getValue(), i++, 1);
                propertiesTable.setValueAt("broknness", i, 0);
                propertiesTable.setValueAt(selectedObject.getProperty("urn:rescuecore2.standard:property:brokenness").getValue(), i++, 1);
                if (MrlBuilding.VIEWER_BUILDINGS_MAP.containsKey(agent)) {
                    MrlBuilding mrlBuilding = MrlBuilding.VIEWER_BUILDINGS_MAP.get(agent).get(selectedObject.getID());
                    ...
                    propertiesTable.setValueAt("CivilianPossibly", i, 0);
                    propertiesTable.setValueAt(mrlBuilding.getCivilianPossibly(), i++, 1);
                    propertiesTable.setValueAt("CivilianPossibly", i, 0);
                    propertiesTable.setValueAt(mrlBuilding.getCivilianPossibleValue(), i++, 1);
                ...
                if (MrlRoad.VIEWER_ROADS_MAP.containsKey(agent)) {
                    MrlRoad mrlRoad = MrlRoad.VIEWER_ROADS_MAP.get(agent).get(selectedObject.getID());
...
//        viewer.setPreferredSize(new Dimension(900, 500));
        ...
        addAllInCombo(agentCombo, new ArrayList<StandardEntity>(model.getEntitiesOfType(StandardEntityURN.FIRE_BRIGADE)));
        addAllInCombo(agentCombo, new ArrayList<StandardEntity>(model.getEntitiesOfType(StandardEntityURN.POLICE_FORCE)));
        addAllInCombo(agentCombo, new ArrayList<StandardEntity>(model.getEntitiesOfType(StandardEntityURN.AMBULANCE_TEAM)));
        JScrollPane c = new JScrollPane(agentCombo);
//        c.setMinimumSize(new Dimension(80,80));
//        c.setMaximumSize(new Dimension(80,80));
        c.setBorder(null);
        JScrollPane s = new JScrollPane(propertiesTable);
        s.setMinimumSize(new Dimension(80, 195));
//        s.setBorder(new EtchedBorder(EtchedBorder.RAISED));
        ...
        BUILDINGS = new FastMap<EntityID, StandardEntity>();
            ...
            menuItem.addMouseListener(new MouseListener() {
                ...
                public void mouseEntered(MouseEvent e) {
                    if (e.getSource() instanceof JMenuItem) {
                        if (!innerPopUp.isShowing()) {
//                            innerPopUp.show(checkBoxMenuItem, e.getX() + 10, e.getY());
                            int over = 0;
                            if (innerPopUp.getComponents().length > 0) {
                                over = menuItem.getText().length() * 10;
                            }
                            innerPopUp.show(checkBoxMenuItem, menuItem.getLocation().x + over, menuItem.getLocation().y);
                        }
                    }
                }
                ...
                public void mouseExited(MouseEvent e) {
                    if (e.getSource() instanceof JMenuItem) {
                        if (innerPopUp.isShowing() && !innerPopUp.contains(e.getPoint())) {
        ...
        JScrollPane t = new JScrollPane(menu);
        t.setBorder(new EtchedBorder(EtchedBorder.RAISED));
        t.setMinimumSize(new Dimension(80, 500));

        JSplitPane pane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
        pane.setDividerSize(5);
        pane.setDividerLocation(195);
        ...
        idTextField.setPreferredSize(new Dimension(60, 25));
        ...
        checkIdTextField.setPreferredSize(new Dimension(60, 25));
        checkIdTextField.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent actionEvent) {
                JTextField field = (JTextField) actionEvent.getSource();
                try {
                    MrlPlatoonAgent.CHECK_ID2 = new EntityID(Integer.parseInt(field.getText()));
                } catch (Exception ignore) {
                }
            }
        });
        JTextField xyTextField = new JTextField();
        xyTextField.setPreferredSize(new Dimension(60, 25));
        xyTextField.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                ...
                try {
                    ...
                    setXY(Integer.parseInt(s[0]), Integer.parseInt(s[1]));
                } catch (Exception ignore) {
                }
            }
        });
        ...
        bPanel.setPreferredSize(new Dimension(600, 25));
        ...
        viewer.addViewListener(new ViewListener() {
            ...
            public void objectsClicked(ViewComponent view, List<RenderedObject> objects) {
                ...
                if (objects.isEmpty()) {
                    ...
                    selectedObject = null;
                    MrlPlatoonAgent.CHECK_ID = null;
                    MrlPlatoonAgent.CHECK_ID2 = null;
                    selectObject(null);
                    viewer.repaint();
                }
                if (objects.size() == 1) {
...
//        paintField.updateUI();
    ...
    private void expandNode(DefaultMutableTreeNode node) {
        ...
        if (!node.children().hasMoreElements()) {
            for (Method m : nodeInfo.getClass().getMethods()) {
                if (m.getAnnotation(AttributeField.class) != null) {

                    DefaultMutableTreeNode childNode = new DefaultMutableTreeNode(m.getName());
                    node.add(childNode);
                    Object obj;
                    try {
                        obj = m.invoke(nodeInfo);
                        if (obj != null) {
                            if (obj instanceof ArrayList) {
                                for (Object o : (ArrayList) obj) {
                                    childNode.add(new DefaultMutableTreeNode(o));
                                }
                            } else {
                                childNode.add(new DefaultMutableTreeNode(obj));
                            }
                        }
                    } catch (Exception ignore) {
                    }
