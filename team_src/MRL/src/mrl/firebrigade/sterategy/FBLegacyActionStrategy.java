...
//        }
    ...
    public void setDirection() {
        ...
        List<FireCluster> civilianClusters = world.getCivilianClusterManager().getClusters();
        for (Cluster fireCluster : fireClusters) {
            for (Cluster civCluster : civilianClusters) {
                ...
                if (clusterValue > maxValue) {
                    maxValue = clusterValue;
                    bestCluster = civCluster;

                }
            }
            try {
                self.getFireBrigadeDirectionManager().getBuildingInDirectionCivilianCluster(bestCluster.getCenter(), fireCluster.getConvexHullObject());
            } catch (NullPointerException e) {
                self.getFireBrigadeDirectionManager().getBuildingInDirectionWorldCentre(fireCluster.getConvexHullObject());
            }
            self.setConvexObject(fireCluster.getConvexHullObject());
        }
    }
...
//            isThinkTimeOver("extinguish myself");
        ...
        if (MRLConstants.LAUNCH_VIEWER && target != null) {
...
//                    MrlConvexHullLayer.STAND.remove(world.getSelf().getID());
                    List<StandardEntity> stands = new ArrayList<StandardEntity>();
                    stands.add(targetToGo);
...
//                   }
    ...
    private StandardEntity chooseBestWayToFire() {
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            Set<StandardEntity> inRangeSet = new HashSet<StandardEntity>(inRange);
