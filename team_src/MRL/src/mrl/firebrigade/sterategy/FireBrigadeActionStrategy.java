...
//                    fireBrigade = (FireBrigade) entity;
//                    if (fireBrigade.isPositionDefined() && fireBrigade.getPosition(world) instanceof Hydrant) {
//                        if (fireBrigade.getID().getValue() < self.getID().getValue()) {
//                            havePermission = false;
//                            break;
//                        }
//                    }
...
//                    self.sendRestAct(world.getTime());
    ...
    protected void updateViewer(MrlBuilding targetBuilding) throws TimeOutException {
        self.isThinkTimeOver("selectBuilding");
        if (MRLConstants.LAUNCH_VIEWER) {
            if (targetBuilding != null) {

                MrlHumanLayer.TARGETS_TO_GO_MAP.put(world.getSelf().getID(), targetBuilding.getID());
                MrlHumanLayer.FIRE_BRIGADE_TARGETS_TO_GO_MAP.put(world.getSelf().getID(), targetBuilding.getID());

                MrlConvexHullLayer.MY_TARGET.remove(self.getID());
                List<Building> trgt = new ArrayList<Building>();
                trgt.add(targetBuilding.getSelfBuilding());
