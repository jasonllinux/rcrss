...
import mrl.viewer.layers.MrlBuildingValuesLayer;
...
//                    b.BUILDING_VALUE /= 1.54;
    ...
    public void setCenterDirection() {
        MrlFireBrigade me = (MrlFireBrigade) world.getPlatoonAgent();
        for (Cluster fireCluster :(List<FireCluster>)world.getFireClusterManager().getClusters()) {
            me.getFireBrigadeDirectionManager().setCenterBasedDirection(fireCluster.getConvexHullObject());
        }
    }
    public void setDirection() {
        ...
        for (Cluster fireCluster : (List<FireCluster>)world.getFireClusterManager().getClusters()) {
            for (Cluster civCluster : (List<CivilianCluster>)world.getCivilianClusterManager().getClusters()) {
                double clusterValue = civCluster.getValue();
                double distance = Util.distance(fireCluster.getConvexHullObject().getConvexPolygon(), civCluster.getConvexHullObject().getConvexPolygon());
                double value = clusterValue;
                if (value > maxValue) {
                    maxValue = value;
                    bestCluster = civCluster;
            ...
            MrlFireBrigade me = (MrlFireBrigade) world.getPlatoonAgent();
            try {
                me.getFireBrigadeDirectionManager().getBuildingInDirectionCivilianCluster(bestCluster.getCenter(), fireCluster.getConvexHullObject());
            } catch (NullPointerException e) {
                me.getFireBrigadeDirectionManager().getBuildingInDirectionWorldCentre(fireCluster.getConvexHullObject());
            }
            me.setConvexObject(fireCluster.getConvexHullObject());
        }
    }
    ...
    private MrlBuilding getOverallBestBuildings(List<MrlBuilding> burningBuildings) {
        ...
        for (MrlBuilding b : burningBuildings) {
            ...
            if (calculateValueForLessValueBuildings(sortedBuildings, lessValueBuildings, b, distanceNormalizer)) continue;

            if (calculateValueForHighValueBuildings(sortedBuildings, highValueBuildings, b, distanceNormalizer)) continue;
...
//                highValueBuildings = directionManager.getHighValueBuildings();
                if (MRLConstants.LAUNCH_VIEWER) {
                    for (MrlBuilding b : highValueBuildings) {
                        borderDirectionBuildings.add(b.getSelfBuilding());
                    }
                }
            } else {
//                highValueBuildings = directionManager.getHighValueBuildings();
...
//                }
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            MrlConvexHullLayer.BORDER_DIRECTION_BUILDINGS.put(world.getSelfHuman().getID(), borderDirectionBuildings);
        }
