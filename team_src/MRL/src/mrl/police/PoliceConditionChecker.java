...
public class PoliceConditionChecker {
    ...
    private boolean isReachable(Target target) {
        ...
        } else if (world.getEntity(target.getId()) instanceof Human) {
            if (world.getChanges().contains(target.getId())) {
                Human human = (Human) world.getEntity(target.getId());
        ...
        if (isThereUnknownRoad) {
            ...
            return !clearHelper.shouldCheckForBlockadesOnWay(pathToGo, target.getId());
        }
