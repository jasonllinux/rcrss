...
public abstract class DefaultActionStrategy implements IPoliceActionStrategy {
    ...
    protected DefaultActionStrategy(MrlWorld world, ClearHelper clearHelper, PoliceMessageHelper policeMessageHelper, PoliceForceUtilities utilities, PoliceConditionChecker conditionChecker) {
        this.world = world;
...
//        me.possibleBuildingSearchDecisionMaker.setSearchInPartition(true);
        ...
        me.heatTracerDecisionMaker.setSearchInPartition(true);
...
//        me.possibleBuildingSearchManager = new PossibleBuildingSearchManager(world, me, me.possibleBuildingSearchDecisionMaker, me.senseSearchStrategy);
    ...
    public void scapeOut() throws CommandException {
        ...
        if (shouldScape()) {
            ...
            clearHelper.clearBlockadesInRange(new Pair<Integer, Integer>(selfHuman.getX(), selfHuman.getY()), 1000);
    ...
    private List<EntityID> randomBuildingSearch() {
        ...
        if (paths.isEmpty()) {
            Partition partition = world.getPartitionManager().findHumanPartition(selfHuman);
            if (partition == null) {
                paths = world.getPaths();
            } else {
                paths = new ArrayList<Path>(partition.getPaths());
            }
        }
...
//        }
        ...
        if (targetRoad == null || world.getSelfPosition().equals(targetRoad)) {
            ...
            Area middleRoad = paths.get(index).getMiddleRoad();
            targetRoad = (Road) middleRoad;
            targetID = middleRoad.getID();
