...
import mrl.viewer.layers.MrlBuildingLayer;
...
public abstract class CivilianSearchDecisionMaker extends SearchDecisionMaker {
    ...
    public static Map<EntityID, String> AGENT_TARGET = new HashMap<EntityID, String>();                         //map(agentID => targetID)
    //<======
    ...
    public void update() {
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            EntityID agentID = world.getPlatoonAgent().getID();
            if (!POSSIBLE_BUILDINGS.containsKey(agentID)) {
                MrlBuildingLayer.POSSIBLE_BUILDINGS.put(agentID, new ArraySet<EntityID>());
                AGENT_TARGET.put(agentID, "Civilian[" + civilianInProgress + "],Building[" + buildingInProgress + "]");
            }
            MrlBuildingLayer.POSSIBLE_BUILDINGS.put(agentID, new ArraySet<EntityID>(shouldDiscoverBuildings));
        }
    }
    ...
    private void removeUnreachableBuildings() {
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            EntityID agentID = world.getPlatoonAgent().getID();
            UNREACHABLE_POSSIBLE_BUILDINGS.put(agentID, new HashSet<EntityID>(toRemove));
        }
    }
