...
//        possibleBuildings.clear();
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            try {
                if (!MrlBuildingLayer.POSSIBLE_BUILDINGS.containsKey(world.getPlatoonAgent().getID())) {
                    MrlBuildingLayer.POSSIBLE_BUILDINGS.put(world.getPlatoonAgent().getID(), new ArraySet<EntityID>());
                }
                MrlBuildingLayer.POSSIBLE_BUILDINGS.get(world.getPlatoonAgent().getID()).clear();
                MrlBuildingLayer.POSSIBLE_BUILDINGS.get(world.getPlatoonAgent().getID()).addAll(possibleBuildings);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
