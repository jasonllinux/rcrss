...
//                    }
        ...
        for (StandardEntity entity : getEntities()) {
            ...
            if (burningBuilding.getEstimatedFieryness() == 1) dangerBuildings.add(burningBuilding);
            if (burningBuilding.getEstimatedFieryness() == 2) dangerBuildings.add(burningBuilding);
...
//            totalArea = building.getSelfBuilding().getShape().getBounds().getWidth() * building.getSelfBuilding().getShape().getBounds().getHeight();
    ...
    public boolean hasBuildingInDirectionOf(Point center, boolean limitDirection, boolean useAllFieryness) {
...
//            for (StandardEntity entity : entities) {
    ...
    public List<MrlBuilding> findBuildingsInDirectionOf(Point targetPoint, boolean limitDirection, boolean useAllFieryness) {
    ...
    private boolean isOldCandidate(MrlBuilding b) {
        return !(b.getEstimatedFieryness() == 3 && b.getEstimatedTemperature() < 150);
    }
    ...
    private void setTriangle(Point targetPoint, boolean limitDirection) {
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            MrlConvexHullLayer.CENTER_POINT.put(world.getSelf().getID(),new Pair<Point, ConvexObject>(targetPoint,this.convexObject));
        }
