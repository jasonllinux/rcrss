...
//        clusterSet.clear();
        ...
        for (MrlBuilding building : world.getMrlBuildings()) {
            if (MRLConstants.LAUNCH_VIEWER) {
                building.BUILDING_VALUE = Double.NaN;
            }
...
//        List<Pair<Point2D, String>> clusterConditions = new ArrayList<Pair<Point2D, String>>();
        for (FireCluster c : clusters) {
            c.updateConvexHull();
            c.setAllEntities(world.getAreasInShape(c.getConvexHullObject().getConvexPolygon()));//Mostafa
        }
        ...
        if (MRLConstants.LAUNCH_VIEWER) {
            MrlConvexHullLayer.IGNORED_BORDER_BUILDINGS.put(world.getSelf().getID(), ignoredBorderBuildings);
        }
        /*if (MRLConstants.LAUNCH_VIEWER) {
            MrlConvexHullLayer.FIRE_CLUSTER_CONDITIONS.put(world.getSelf().getID(), clusterConditions);
        }*/
    }
