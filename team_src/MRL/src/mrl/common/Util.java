...
//        return Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
    ...
    public static List<rescuecore2.misc.geometry.Point2D> intersections(Polygon polygon, rescuecore2.misc.geometry.Line2D line) {
        ...
        for (rescuecore2.misc.geometry.Line2D ln : polyLines) {
            rescuecore2.misc.geometry.Point2D intersectPoint = GeometryTools2D.getIntersectionPoint(line, ln);
            if (contains(ln, intersectPoint, 5)) {
    ...
    public static boolean hasIntersection(Polygon polygon, rescuecore2.misc.geometry.Line2D line) {
        List<rescuecore2.misc.geometry.Line2D> polyLines = getLines(polygon);
        for (rescuecore2.misc.geometry.Line2D ln : polyLines) {
            if (math.geom2d.line.Line2D.intersects(
                    new math.geom2d.line.Line2D(line.getOrigin().getX(), line.getOrigin().getY(), line.getEndPoint().getX(), line.getEndPoint().getY()),
                    new math.geom2d.line.Line2D(ln.getOrigin().getX(), ln.getOrigin().getY(),
                            ln.getEndPoint().getX(), ln.getEndPoint().getY()))) {
                return true;
            }
        }
        return false;
