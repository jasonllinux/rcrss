...
public class Partition {
    ...
    private List<Pair<Line2D, Partition>> neighbours = new ArrayList<Pair<Line2D, Partition>>();
    ...
    private StandardEntity findCenterEntity() {
        int maxDistanceInPartition = findMaxDistanceInPartition();
        int minDistance = Integer.MAX_VALUE;
        int tempDistance;
        StandardEntity nearestEntityToCenter = null;
        for (StandardEntity entity : world.getObjectsInRange(center.first(), center.second(), maxDistanceInPartition)) {
    ...
    private int findMaxDistanceInPartition() {
        int maxDistance = Integer.MIN_VALUE;
        int tempDistance;
        if (polygon != null) {
            for (int i = 0; i < polygon.npoints; i++) {
                for (int j = 0; j < polygon.npoints; j++) {
                    tempDistance = Util.distance(polygon.xpoints[i], polygon.ypoints[i], polygon.xpoints[j], polygon.ypoints[j]);
            ...
            for (MrlBuilding b1 : buildings) {
                for (MrlBuilding b2 : buildings) {
                    tempDistance = world.getDistance(b1.getSelfBuilding(), b2.getSelfBuilding());
                    if (tempDistance > maxDistance) {
                        maxDistance = tempDistance;
                    }
...
//            id=buildings.get(0).getID();
//        }else if(!roads.isEmpty()){
//            id=roads.get(0).getID();
//        }else {
//            id=new EntityID(new Random(100).nextInt());
//        }
//
//
//        int distance=Integer.MAX_VALUE;
//        int temp;
//        for (StandardEntity entity:world.getObjectsInRange(center.first(),center.second(), world.getPlatoonAgent().viewDistance)){
//             temp=getDistanceToCenter(entity);
//            if(temp<distance){
//               id=entity.getID();
//            }
//
...
//        }
    ...
    public void addNeighbours(Line2D commonLine, Partition partition) {
        this.neighbours.add(new Pair<Line2D, Partition>(commonLine, partition));
    }

    public List<Pair<Line2D, Partition>> getNeighbours() {
        return neighbours;
    }
...
//
        ...
        for (Pair<Line2D, Partition> neighbourPartition : partition.getNeighbours()) {
        ...
        partition.getNeighbours().removeAll(toRemove);
        toRemove.clear();
        ...
        for (Pair<Line2D, Partition> neighbourPartition : neighbours) {
        ...
        neighbours.addAll(partition.getNeighbours());
...
*/
    ...
    private void refreshNeighbour(Partition oldPartition, Partition newPartition) {
        ...
        for (Pair<Line2D, Partition> neighbour : neighbours) {
