...
//        assignmentMethod = new GreedyAssignment();
    ...
    protected OperationMode partitionDecideOperationMode(int numberOfHealthyAgents, int numberOfAllAgents) {
        OperationMode decidedOperationMode = null;
    ...
    public Partition findNearestNeighbourPartition(Partition currentPartition, List<Partition> partitions) {
        ...
        for (Partition partition : partitions) {
            ...
            if (!partition.isDead()
                    && ((unvisitedBuilding != null && !unvisitedBuilding.isEmpty()) || (victims != null && !victims.isEmpty()))
    ...
    protected void updateViewer() {
        if (MRLConstants.LAUNCH_VIEWER) {
            ...
            MrlClusterLayer.PARTITIONS_MAP.put(world.getSelf().getID(), getPartitions());
            MrlClusterLayer.HUMAN_PARTITION_MAP.put(world.getSelf().getID(), findHumanPartition(world.getSelfHuman()));
            if (findHumanPartition(world.getSelfHuman()) == null) {
                if (findHumanPartition(world.getSelfHuman()) == null) {
                    System.out.println(world.getSelf().getID() + " I have no partition in time :" + 0);
//        }
                } else {

                    System.out.println(world.getSelf().getID() + " my Partition is:" + findHumanPartition(world.getSelfHuman()).getId() + " NumberOfNeeded:" + findHumanPartition(world.getSelfHuman()).getNumberOfNeededPFs());
                }
            }
            MrlSubClusterLayer.Partitions = getPartitions();
            MrlPartitionsLayer.PARTITIONS = getPartitions();
            MrlRendezvousLayer.partitions = getPartitions();
        }
...
//            System.out.println((world.getTime() + " self: " + world.getSelf().getID() + " agentID: " + agents.get(assignment[i]).getID() + " partition: " + deflatedPartitions.get(i).getId()));
    ...
    @Override
    public List<Partition> findHumanPartitionsMap(Human human) {
