...
public class DefaultWorldModel<T extends Entity> extends AbstractWorldModel<T> {
	 ...
	 * 
	 * @param clazz
	 *            The class of objects that are allowed in this world model.
	 *            This approach is a workaround for the limitations of Java
	 *            generics.
	 */
	...
	/**
	 * POSEIDON TEAM BASE PART
	 * 
	 * @param integer
	 * @return returns the EntityID by giving the Value(Integer Value) of an
	 *         entity
	 */
	public final Entity getEntityByInt(int id) {
		return getEntity(new EntityID(id));
	}

	public boolean exist(int id) {
		return entities.containsKey(new EntityID(id));
	}
}
