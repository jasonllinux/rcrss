 ...
 */
public abstract class StandardAgent<E extends StandardEntity> extends
		AbstractAgent<StandardWorldModel, E> {
	@Override
	...
	protected void sendClearArea(int time, int destinationX, int destinationY) {
		send(new AKClearArea(this.getID(), time, destinationX, destinationY));
	}
	...
	public void sendSpeak(int time, int channel, byte[] data) {
		agent.Agent.counterForBusiness += data.length;
