...
public class HungarianAlgorithm {
	...
	public double findLargest // Finds the largest element in a positive
	// array.
	(double[][] array)
	...
	public double[][] copyOf // Copies all elements of an array to a new
	// array.
	(double[][] original) {
	...
	public int[] hgAlgorithm(double[][] array, String sumType) {
		...
		int[] assignment = new int[array.length]; // Create the returned
		// array.
		...
		// public double hgAlgorithm (double[][] array, String sumType)
	...
	public int hg_step2(int step, double[][] cost, int[][] mask,
	...
	public int hg_step4(int step, double[][] cost, int[][] mask,
	...
	public int hg_step5(int step, int[][] mask, int[] rowCover, int[] colCover,
			int[] zero_RC) {
		// What STEP 5 does:
	...
	public int hg_step6(int step, double[][] cost, int[] rowCover,
	...
	public double[][] to2DArray(ArrayList<PathArray> pa,
			ArrayList<PoliceForce> modelPolices) {
		double[][] double2D = new double[modelPolices.size()][pa.size()];
		for (int i = 0; i < modelPolices.size(); i++)
			for (int j = 0; j < pa.size(); j++) {
				double2D[i][j] = Math.hypot(
						modelPolices.get(i).firstX - pa.get(j).centerX,
						modelPolices.get(i).firstY - pa.get(j).centerY);
			}
		return double2D;
	}

	public int getMyPathNum(int myNum) {
		return matched[myNum];
	}

	int[] matched = null;

	public void execute(ArrayList<PathArray> pa,
			ArrayList<PoliceForce> modelPolices) {
		double[][] array = to2DArray(pa, modelPolices);
		matched = hgAlgorithm(array, "min");
	}

	int[] joined = null;
	public static int time = -1;

	public int getPoliceGroupNum(int num) {
		return joined[num];
	}

	public void matching(ArrayList<GroupForPolice> modelGroups,
			ArrayList<PoliceForce> modelPolices, StandardWorldModel model) {
		double[][] array = to2DArrayFormatchingGroups(modelGroups,
				modelPolices, model);
		joined = hgAlgorithm(array, "min");
		for (int i = 0; i < modelPolices.size(); i++) {
			if (joined[i] >= modelGroups.size())
				joined[i] = -1;
		}
	}

	public double[][] to2DArrayFormatchingGroups(
			ArrayList<GroupForPolice> modelGroups,
			ArrayList<PoliceForce> modelPolices, StandardWorldModel model) {

		double[][] double2D = new double[modelPolices.size()][Math.max(
				modelGroups.size(), modelPolices.size())];

		for (int i = 0; i < double2D.length; i++)
			for (int j = 0; j < double2D[i].length; j++)
				double2D[i][j] = Integer.MAX_VALUE;

		for (int i = 0; i < modelPolices.size(); i++) {
			Area areaOfpolice = (Area) model.getEntity(modelPolices.get(i)
					.getPosition());
			for (int j = 0; j < modelGroups.size(); j++) {
				Area areaOfGroup = (Area) model.getEntity(((Human) modelGroups
						.get(j).StuckAgents.toArray()[0]).getPosition());
				double dist = Math.hypot(
						areaOfpolice.getX() - areaOfGroup.getX(),
						areaOfpolice.getY() - areaOfGroup.getY());
				if (modelPolices.get(i).stuckTarget != null){
					for (Human hu : modelGroups.get(j).StuckAgents)
						if (hu.getPosition().getValue() == modelPolices.get(i).stuckTarget
								.getID().getValue()) {
							dist *= 0.75;
							break;
						}
